...
1. **Preflight**:
   ...
   - **Target Project Script Validation (Programmatic & Interactive)**: Deduces the recommended sequence of commands for the target project (such as setup, install, dev server start, and build) **100% programmatically and dynamically** by inspecting its `package.json` and files. To prevent startup delays, timeouts, and hallucinations, `agy` is **never** invoked for target project validation. After displaying the recommended commands, NodePi pauses execution and prompts the user whether they wish to execute the sequence sequentially in the target project via a shell with inherited output (`stdio: 'inherit'`).
...
**AI Fallback Mode**: If `agy` fails to execute, times out (15 seconds limit), or returns invalid JSON, the CLI does not crash. It automatically falls back to an interactive prompt. The prompt extracts the `"scripts"` object from the dependency's `package.json` and lets the user manually select the compilation script (or choose "None"). It then prompts the user for the output directory, offering defaults like `dist`, `lib`, or `build`.

**Payload Optimization**: To prevent `agy` timeouts and context overload due to large dependency sizes, the `package.json` data sent to the AI prompt is simplified by extracting only `"name"`, `"scripts"`, and dependency keys (omitting full dependency objects and nested versions). This reduces prompt payload size by 80%, improving response speeds.
